<template>
    <Navbar>
        <PageTitle>
            ...
            <Breadcrumb><span>Print Queue</span></Breadcrumb>
        </PageTitle>
        <RightSide></RightSide>
    </Navbar>
    <table
    ...
    >
        ...
        <tbody v-for="(order, index) in orders" :key="order.order_id">
            <tr class="border-b border-gray-100">
                ...
                <td class="p-6 justify-start grid grid-cols-2 gap-2">
                    <Link
                        v-if="order.order_status === 3"
                        :href="'/printing/' + order.order_id"
                        method="put"
                    ...
                    <Link
                        ...
                        :href="'/order/' + order.order_id + '/file'"
                        class="col-span-1 px-3 py-2 bg-indigo-500 text-white rounded-md text-sm font-semibold text-center"
                        >View ({{ files[index] }}) File</Link
                    >
                    <Link
                        v-if="order.order_status === 4"
                        :href="'/complete/' + order.order_id"
                        method="put"
                        class="px-3 py-2 bg-emerald-400 text-white rounded-md text-sm font-semibold text-center"
                        >Complete Order</Link
                    >
                </td>
            </tr>
